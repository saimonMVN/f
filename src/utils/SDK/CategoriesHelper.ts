...
export default class CategoriesHelper {
    ...
    static pushCategoriesIntoMenuData<T1 extends IFormatCategoriesMenuType>(menuData: T1[], productCategories: ProductCategory[] | undefined): T1[] {
    ...
    private static getCategoriesParents(categories: ProductCategory[]): ProductCategory[] {
    ...
    private static modifyMedusaCategoriesAsMenu(filtersCategories: ProductCategory[]): ISubMenu[] {
        if (filtersCategories) {
            return filtersCategories.map(category => {
                ...
                if (category.category_children && category.category_children.length > 0) {
                    mappedCategory.subMenu = this.modifyMedusaCategoriesAsMenu(category.category_children);
                }
    ...
    private static modifyMedusaCategoriesAsTemplate(categories: ProductCategory[]): Category[] {
        ...
        return categories.map(category => {
            ...
            if (category.category_children && category.category_children.length > 0) {
                mappedCategory.children = this.modifyMedusaCategoriesAsTemplate(category.category_children);
            }
            return mappedCategory;
        })
    }
