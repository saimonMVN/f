...
const ShippingAddress:  React.FC<MyInformationProps> = ({ customer }) => {
  ...
  const { setSavedAddress } = useCheckout()
  ...
  useEffect(()=>{
    // if none selected we will select the first
    if(shipping_addresses.length) {
      setSelected(shipping_addresses[0])
      setSavedAddress(shipping_addresses[0])
    }
   }, [shipping_addresses])

  const handleSelect = (id: string) => {
    const savedAddress = shipping_addresses.find((a) => a.id === id)
    if (savedAddress) {
